...
class PushViewController: UIViewController, LFLiveSessionDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        LottieAnimationManager.shared.createlottieAnimation(name: "loading", view: self.view, animationSpeed: 4, isPulling: false)
    ...
    @objc func didTappedRecordButton(_ button: UIButton) {
        ...
        if record.isRecording {
            RecordManager.record.stopRecording(record, self) { result in
                switch result {
                case .success(_):
                    LottieAnimationManager.shared.createlottieAnimation(name: "Success", view: self.view, animationSpeed: 4, isPulling: false)
                case .failure(_):
                    LottieAnimationManager.shared.createlottieAnimation(name: "Fail", view: self.view, animationSpeed: 4, isPulling: false)
                }
            }
        } else {
        ...
        if recordingSeconds == 10 {
            ...
            RecordManager.record.stopRecording(record, self) { [weak self] result in
                ...
                case .success(_):
                    ...
                    LottieAnimationManager.shared.createlottieAnimation(name: "Success", view: self?.view ?? UIView(), animationSpeed: 4, isPulling: false)
                case .failure(_):
                    LottieAnimationManager.shared.createlottieAnimation(name: "Fail", view: self?.view ?? UIView(), animationSpeed: 4, isPulling: false)
                }
            }
        }
    }
